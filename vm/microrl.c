...
static void hist_erase_older(ring_history_t *pThis) {
    ...
    if (new_pos >= _RING_HISTORY_LEN) new_pos = new_pos - _RING_HISTORY_LEN;
...
static int hist_is_space_for_new(ring_history_t *pThis, int len) {
    if (pThis->ring_buf[pThis->begin] == 0) return true;
    if (pThis->end >= pThis->begin) {
        if (_RING_HISTORY_LEN - pThis->end + pThis->begin - 1 > len)
            return true;
    } else {
        if (pThis->begin - pThis->end - 1 > len) return true;
    }
    return false;
}
...
static void hist_save_line(ring_history_t *pThis, char *line, int len) {
    if (len > _RING_HISTORY_LEN - 2) return;
    while (!hist_is_space_for_new(pThis, len)) { hist_erase_older(pThis); }
    // if it's first line
    if (pThis->ring_buf[pThis->begin] == 0) pThis->ring_buf[pThis->begin] = len;
    ...
    if (pThis->end >= _RING_HISTORY_LEN) pThis->end -= _RING_HISTORY_LEN;
...
static int hist_restore_line(ring_history_t *pThis, char *line, int dir) {
    ...
    while (pThis->ring_buf[header] != 0) {
        ...
        if (header >= _RING_HISTORY_LEN) header -= _RING_HISTORY_LEN;
        cnt++;
    }
...
static int split(microrl_t *pThis, int limit, char const **tkn_arr) {
    ...
    while (1) {
        ...
        while ((pThis->cmdline[ind] == '\0') && (ind < limit)) { ind++; }
        if (!(ind < limit)) return i;
        tkn_arr[i++] = pThis->cmdline + ind;
        if (i >= _COMMAND_TOKEN_NMB) { return -1; }
...
//*****************************************************************************
inline static void terminal_newline(microrl_t *pThis) { pThis->print(ENDL); }
...
static int common_len(char **arr) {
    ...
    for (i = 0; i < shortlen; ++i)
        for (j = 0; arr[j] != 0; ++j)
            if (shortest[i] != arr[j][i]) return i;
...
static void microrl_get_complite(microrl_t *pThis) {
    ...
    if (pThis->cmdline[pThis->cursor - 1] == '\0') tkn_arr[status++] = "";
    ...
    if (compl_token[0] != NULL) {
        ...
        if (len) {
            ...
            if (compl_token[1] == NULL) microrl_insert_text(pThis, " ", 1);
        }
        terminal_reset_cursor(pThis);
        terminal_print_line(pThis, 0, pThis->cursor);
    }
...
#ifdef _USE_ESC_SEQ
    if (pThis->escape) {
        if (escape_process(pThis, ch)) pThis->escape = 0;
    } else {
#endif
        switch (ch) {
            //-----------------------------------------------------
        ...
        case KEY_CR:
            ...
            break;
        case KEY_LF:
            if (pThis->tmpch == KEY_CR) new_line_handler(pThis);
            break;
        ...
        case KEY_LF:
            ...
            break;
        case KEY_CR:
            if (pThis->tmpch == KEY_LF) new_line_handler(pThis);
            break;
#else
        case KEY_CR:
            break;
        case KEY_LF:
            ...
            //-----------------------------------------------------
            case KEY_NAK:  // ^U
                while (pThis->cursor > 0) { microrl_backspace(pThis); }
                terminal_print_line(pThis, 0, pThis->cursor);
                break;
            //-----------------------------------------------------
            case KEY_VT:  // ^K
...
#ifdef _USE_CTLR_C
            case KEY_ETX:
                if (pThis->sigint != NULL) pThis->sigint();
                break;
#endif
            //-----------------------------------------------------
            default:
